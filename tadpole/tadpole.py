...
class Tadpole(IterableDataset):
    def __init__(self, datapath, name="TADPOLE", device='cpu'):
        ...
        self.score_file = self.datapath + '/y.mat'
        ...
        # Fill labels
        score = loadmat(self.score_file)['Y']
        max_value_perid = score.max(1)
        labels = 0 * max_value_perid  #no AD
        labels[max_value_perid >= 8] = 1  #AD
    ...
    def visualize(self, traj, plot_path=None, slices=(50, 60, 50), **kwargs):
        raise NotImplemented("Visualization for Tadpole is not implemented.")
        try:
            traj = traj.cpu().numpy()
        except:
            pass
