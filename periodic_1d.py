...
def make_data(ts,
    ...
    # n_diff_models: how many models to generate from the same initial value
    ...
    if n_diff_models == 2:
        amp_coef = np.array((-1, 1, -1, 1))
        freq_coef = np.array((2, 2, -1, -1))
    else:
        amp_coef = np.random.rand(n_diff_models) * 2 - 1  #runif(0, 1)
        freq_coef = np.random.rand(n_diff_models) * 2 - 1  #runif(0, 1)
    amp_coef = np.resize(amp_coef, ys.shape[0])  # fit it to size of bs
    freq_coef = np.resize(freq_coef, ys.shape[0])  # fit it to size of bs
    ...
    for j in range(1, len(ts)):
        ...
        phi = phi + 2 * np.pi * freq * dt * freq_coef  #integrate to get phase
